...
router.get("/forgot-pass2", (req, res) => {
  res.render("account/forgot-pass-2", {
    q1: question[0],
    q2: question[1],
    q3: question[2],
    message: req.flash("erpass2"),
  });
});
...
router.post("/forgot-pass2", (req, res) => {
  ...
  db.all(sql, [username], (err, row) => {
    if (err) {
      ...
      question = [row[0].secQues1, row[0].secQues2, row[0].secQues3];
      answer = [row[0].secAnsw1, row[0].secAnsw2, row[0].secAnsw3];
      uN = row[0].username;
      pass = row[0].password;
      res.render("account/forgot-pass-2", {
        q1: question[0],
...
router.post("/forgot-pass3", (req, res) => {
  const sql =
    "SELECT * FROM userAccount WHERE secAnsw1 = ? AND secAnsw2 = ? AND secAnsw3 = ?";
  ...
  db.all(sql, [ans1, ans2, ans3], (err, row) => {
    if (err) {
      console.log(err.message);
    } else if (ans1 == answer[0] && ans2 == answer[1] && ans3 == answer[2]) {
...
router.post("/forgot-changePass3", (req, res) => {
  const sql =
    "UPDATE userAccount SET password = ? WHERE (username = '" + uN + "')";
...
router.post("/changeUsername", (req, res) => {
  const sql =
    "UPDATE userAccount SET username = ? WHERE (username = '" +
...
router.post("/changePass", (req, res) => {
  ...
  bcrypt.hash(password, saltRounds, (err, hash) => {
    if (
      req.body.newPass == req.body.confirmPass &&
      req.body.oldPass == req.session.password
    ) {
      db.run(sql, [hash], (err, row) => {
        if (err) {
          return console.log(err.message);
...
router.post("/changeSec", (req, res) => {
  const sql =
    "UPDATE userAccount SET secQues1= ?, secQues2 = ?, secQues3 = ?,secAnsw1 = ?, secAnsw2 = ?, secAnsw3 = ? WHERE (username = '" +
    req.session.username +
    "')";
...
router.post("/new", (req, res) => {
  const sql_insert =
    "INSERT INTO userAccount(username,password,secQues1,secQues2,secQues3,secAnsw1,secAnsw2,secAnsw3)VALUES(?,?,?,?,?,?,?,?)";
  ...
  bcrypt.hash(password, saltRounds, (err, hash) => {
    if (req.body.cpw == password) {
      db.run(
        sql_insert,
        [username, hash, secq1, secq2, secq3, seca1, seca2, seca3],
        (err, row) => {
          if (err) {
...
router.post("/delete", (req, res) => {
  const sql =
    "DELETE FROM userAccount WHERE username = ? AND username != 'admin'";
  if (
    req.body.delAccount == req.session.password &&
